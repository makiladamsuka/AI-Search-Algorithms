...
class Node:
    ...
    def __init__(self, name, x=0, y=0, heuristic=0):
        ...
        self.edge_labels = {}  # {neighbor_node: action_label}
        self.state = 'empty'  # 'empty', 'source', 'goal', 'visited', 'path'
        self.parent = None
        self.cost = 0  # g(n) - cost from start
        
    def add_neighbor(self, neighbor, weight=99, label=""):
        """
        Add a neighbor with an edge weight and optional action label
        
        Args:
            neighbor (Node): The neighboring node
            weight (float): Weight/cost of the edge (default 99)
            label (str): Optional action description label
        """
