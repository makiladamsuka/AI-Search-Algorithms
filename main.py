...
class GraphVisualizer:
    ...
    def draw_node(self, node):
        """Draw a node with crisp, non-overlapping multiline rendering"""
        colors = {
            'empty': '#ffffff',
            'source': '#ef4444',
            'goal': '#10b981',
        ...
        self.ctx.strokeStyle = '#3b82f6' if is_selected else '#1e293b'
        self.ctx.lineWidth = 4 if is_selected else 3
        self.ctx.stroke()
        
        # Full display name
        full_name = node.custom_name if (hasattr(node, 'custom_name') and node.custom_name) else str(node.name)
        
        # Short ID inside circle (e.g. S0, S1_Batch, X1, G1_Optimal)
        if ':' in full_name:
        ...
        if desc_text:
            self.ctx.font = 'bold 11px Inter, -apple-system, BlinkMacSystemFont, sans-serif'
            
            # Split desc_text into multiline parts if it has ']' or '('
            lines = []
            if ']' in desc_text:
            ...
            pill_w = max_w + 12
            pill_h = (len(lines) * line_height) + 6
            pill_x = node.x - (pill_w / 2)
            pill_y = node.y - radius - pill_h - 6
            
            # Background pill with clean border
            self.ctx.fillStyle = 'rgba(255, 255, 255, 0.96)'
            self.ctx.fillRect(pill_x, pill_y, pill_w, pill_h)
            self.ctx.strokeStyle = 'rgba(30, 41, 59, 0.4)'
            self.ctx.lineWidth = 1.5
            self.ctx.strokeRect(pill_x, pill_y, pill_w, pill_h)
            
            # Draw text lines
            self.ctx.fillStyle = '#0f172a'
            self.ctx.textAlign = 'center'
            self.ctx.textBaseline = 'top'
            for idx, line in enumerate(lines):
                self.ctx.fillText(line, node.x, pill_y + 4 + (idx * line_height))
    ...
    def draw_edge(self, node1, node2, weight):
        ...
        # Show weight label and action text when labels are enabled
        ...
        if should_show_weight:
            ...
            action_label = getattr(node1, 'edge_labels', {}).get(node2, "")
            cost_str = f"{int(weight)}m" if weight == int(weight) else f"{weight}m"
            if action_label:
                if f"{int(weight)}m" in action_label or f"{weight}m" in action_label:
                    text = action_label
                else:
                    text = f"{action_label} ({cost_str})"
            else:
                text = str(int(weight)) if weight == int(weight) else str(weight)
            ...
            # Theme-aware colors
            is_dark = 'dark-mode' in document.body.classList
            bg_color = 'rgba(15, 23, 42, 0.95)' if is_dark else 'rgba(255, 255, 255, 0.96)'
            border_color = 'rgba(148, 163, 184, 0.5)' if is_dark else 'rgba(30, 41, 59, 0.4)'
            text_color = '#ffffff' if is_dark else '#0f172a'
            
            self.ctx.fillStyle = bg_color
            self.ctx.fillRect(mid_x - pill_w / 2, mid_y - pill_h / 2, pill_w, pill_h)
            self.ctx.strokeStyle = border_color
            self.ctx.lineWidth = 1.2
            self.ctx.strokeRect(mid_x - pill_w / 2, mid_y - pill_h / 2, pill_w, pill_h)
    ...
    def on_mouse_down(self, event):
        ...
        if self.current_tool == 'add-node' and node is None:
            ...
            if edge:
                ...
                # If directed edge is in reverse direction, swap them
                if to_node not in from_node.neighbors and from_node in to_node.neighbors:
                    from_node, to_node = to_node, from_node
                
                current_weight = from_node.get_weight(to_node)
                if current_weight == float('inf') and from_node in to_node.neighbors:
                    from_node, to_node = to_node, from_node
                    current_weight = from_node.get_weight(to_node)
                    
                display_val = str(int(current_weight)) if current_weight == int(current_weight) else str(current_weight)
                value = window.prompt(f'Enter edge weight from {from_node.name} to {to_node.name}:', display_val)
                if value is not None:
                    try:
    ...
    def load_graph_from_data(self, data):
        ...
        for node_data in data['graph']['nodes']:
            ...
            for neighbor_name, weight in node_data.get('neighbors', {}).items():
                action_text = ""
                if ' (' in str(neighbor_name) and str(neighbor_name).endswith(')'):
                    parts = str(neighbor_name).split(' (', 1)
                    clean_name = parts[0].strip()
                    action_text = parts[1].rstrip(')').strip()
                else:
                    clean_name = str(neighbor_name).strip()
                
                # Try clean_name first, then original neighbor_name
                neighbor = name_to_node.get(clean_name)
                if not neighbor and isinstance(clean_name, str) and clean_name.isdigit():
                    neighbor = name_to_node.get(int(clean_name))
                elif not neighbor and isinstance(clean_name, int):
                    neighbor = name_to_node.get(str(clean_name))
                elif not neighbor:
                    neighbor = name_to_node.get(neighbor_name)
                
                if neighbor:
                    node.add_neighbor(neighbor, weight, action_text)
        ...
        # Auto-fit entire graph into view
        self.fit_to_graph()
        self.render()
        self.update_graph_stats()
        
    def fit_to_graph(self):
        """Auto-fit all nodes perfectly within canvas view"""
        if not self.nodes:
            return
        ...
        graph_w = (max_x - min_x) + 160
        graph_h = (max_y - min_y) + 160
        
        if graph_w > 0 and graph_h > 0 and self.window_width > 0 and self.window_height > 0:
            scale_x = self.window_width / graph_w
            scale_y = self.window_height / graph_h
            self.zoom = max(0.4, min(1.0, min(scale_x, scale_y) * 0.94))
            self.target_zoom = self.zoom
            
            center_graph_x = (min_x + max_x) / 2
            center_graph_y = (min_y + max_y) / 2
            
            self.view_offset_x = (self.window_width / 2) - (center_graph_x * self.zoom)
            self.view_offset_y = (self.window_height / 2) - (center_graph_y * self.zoom)
